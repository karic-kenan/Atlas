...
class FavoriteArticleUseCaseTest : FunSpec({
    // Setup - mock dependencies
    val mockArticleRepository = mockk<ArticleRepository>()
    val mockUserRepository = mockk<UsersRepository>()

    val useCase = FavoriteArticleUseCase { email, slug ->
        favoriteArticle(mockUserRepository, mockArticleRepository, email, slug)
    }

    test("should favorite article successfully") {
        runTest {
            // Arrange
            ...
            val favoritedArticle = article.copy(
                favorited = true,
                favoritesCount = 1
            )
            ...
            coEvery { mockArticleRepository.favorite(testUserId, testSlug) } returns 1
            coEvery { mockArticleRepository.getBySlug(testSlug) } returns favoritedArticle
            ...
            returnedArticle.favorited shouldBe true
            returnedArticle.favoritesCount shouldBe 1
        }
    }
