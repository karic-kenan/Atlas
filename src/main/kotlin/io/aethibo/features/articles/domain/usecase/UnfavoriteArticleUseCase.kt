...
fun interface UnfavoriteArticleUseCase : suspend (Email, Slug) -> Either<ArticleFailure, Article>
        ...
        val userId = user.id
            ?: raise(
                ArticleFailure.DatabaseError(
                    operation = "unfavorite operation",
                    cause = RuntimeException("User ID is null")
                )
            )

        val favoritesCount = articleRepository.unfavorite(userId, slug)

        article.copy(favorited = false, favoritesCount = favoritesCount)
    }) { exception ->
